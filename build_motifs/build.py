...
def maga_next(self, viz=False):
    ...
    todo_edges = list(self.maga_graph.edges())
    done_edges = list()
    ...
    for (singleton, motif) in tqdm(todo_edges):
        ...
        new_nodeset = self.merge_nodesets(singleton, motif)
        ...
        # draw motif for visual check..
        if viz:
            self.viz_maganode(new_node, new_nodeset)

        # remove merged edge from maga graph
        try:
            self.maga_graph.remove_edge(singleton, motif)
        except nx.exception.NetworkXError:
            pass
        try:
            self.maga_graph.remove_edge(motif, singleton)
        except nx.exception.NetworkXError:
            pass
        ...
        for clust in new_node:
            if clust in self.boring_clusters:
                boring_count += 1
        if boring_count > 3:
            continue
        ...
        if len(new_nodeset) < self.min_edge:
            continue
        else:
            self.maga_graph.add_node(new_node, node_set=new_nodeset)
            self.maga_tree.add_edge(motif, new_node)

        # connect phase
        # for each cluster in the motif, add connection to other clutters
        # e.g. for ABC, look at all singletons adjacent to A, B, and C
        for clust in new_node:
            for nei in self.graph.neighbors(clust):
                self.maga_graph.add_edge(ms.FrozenMultiset([nei]), new_node)
        pass

        # if new motif is similar in size to the old one,
        # kill the old motif
        pre_merge_size = len(self.maga_graph.nodes[motif]['node_set'])
        if len(new_nodeset) >= .8 * pre_merge_size:
            to_kill.append(motif)
            continue

    self.maga_graph.remove_nodes_from(to_kill)
    pass


def maga(self, levels=10):
    maga_graph = nx.relabel_nodes(self.graph,
                                    {n: ms.FrozenMultiset([n]) for n in self.graph})
    ...
    boring_clusters = {c: {'samples': 0.01, 'boring': 0} for c in set(self.labels)}

    maga_tree = nx.DiGraph()
    maga_tree.add_nodes_from((ms.FrozenMultiset([n]) for n in self.graph))

    self.maga_tree = maga_tree
    ...
    self.maga_adj = defaultdict(def_set)
    ...
    for c1, c2, d in maga_graph.edges(data=True):
        ...
        for u, v in d['edge_set']:
            ...
            self.maga_adj[u][self.labels[v]].add(v)
            self.maga_adj[v][self.labels[u]].add(u)

            for node in (u,v):
                clust = self.labels[node]
                if boring_clusters[clust]['samples'] < n_boring_samples:
                    boring_clusters[clust]['samples'] += 1
                    node_id = self.reversed_node_map[node]
                    G = whole_graph_from_node(node_id)
                    if not has_NC_bfs(G, node_id, depth=1):
                        boring_clusters[clust]['boring'] += 1

            u_node = ms.FrozenMultiset([self.labels[u]])
            v_node = ms.FrozenMultiset([self.labels[v]])
    ...
    self.boring_clusters = {clust for clust, counts in boring_clusters.items()\
                            if counts['boring'] / counts['samples'] > .8}
    self.maga_graph = maga_graph
    for l in range(levels):
        print("maga level ", l)
        print("maga nodes ", len(self.maga_graph.nodes()),
                "maga edges ", len(self.maga_graph.edges())
                )
        self.maga_next()
