...
def blob_to_graph(residues, graph, graph_id):
    """
    ...
    :param graph_id: name of graph to add in node attributes
    :return: nx subgraph
    """
...
def all_rna_process(graph_path='../data/carnaval',
    ...
    pool = mlt.Pool()
    if parallel:
        arguments = [(rna, pdb_path, graph_path, dest) for rna in graphs]
        for res in tqdm(pool.imap_unordered(compute_one_rna, arguments), total=len(graphs)):
            failed += res
        print(f'failed on {(failed)} on {len(graphs)}')
        return failed
    for i, rna in tqdm(enumerate(graphs), total=len(graphs)):
        failed_rna = compute_one_rna((rna, pdb_path, graph_path, dest))
        if failed_rna:
            failed += failed_rna
            print(f'failed on {rna}, this is the {failed}-th one on {len(graphs)}')
    pass
...
if __name__ == "__main__":
    ...
    # parallel=False)
    all_rna_process(graph_path='../data/carnaval_2', pdb_path='../data/all_rna_pdb', dest="../data/chunks_nx_2",
                    parallel=False)
    pass
